...
const SESSION_TABS = [
  ...
  },
  {
    ...
    group: "session",
    label: "Log archive",
    hint: "Game.log backups since patch 4.8 (includes 4.9 and 4.10 LIVE). Click one to view everything we can parse from that file.",
...
const TAB_DESCRIPTIONS = {
  ...
  history:
    "Game.log backups since patch 4.8 (includes 4.9 and 4.10 LIVE). Open an archive to review parsed stats from an older play session.",
  "guides-reputation":
...
function buildHistoryArchives() {
  ...
  if (logArchiveLoading) {
    return `<div class="overview-prose"><p>Scanning logbackups since 4.8 (incl. 4.10)…</p></div>`;
  }
  if (logArchiveError) {
...
function buildPatchNotesPanel(data) {
  ...
  const meta = data.meta?.fetchedAt
    ? `<p class="guides-meta muted small">Game patches from RSI (Alpha 4.10 / 4.9 / 4.8.x) and star-citizen.wiki · cached ${escapeHtml(fmtDateTime(data.meta.fetchedAt))}${data.meta.stale ? " (stale)" : ""} · <button type="button" class="link" id="patchNotesRefreshBtn">Refresh patch notes</button></p>`
