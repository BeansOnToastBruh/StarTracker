...
async function main() {
  ...
  assert.strictEqual(matchesCommodityFilter({ ...sample, isIllegal: 1, priceBuy: 100 }, "illegal"), true);
  assert.strictEqual(matchesCommodityFilter(sample, "all"), true);
  ...
  const routes = await getSmugglerRoutes();
  ...
  const refineryIntel = require("../electron/refineryIntel");
  refineryIntel.init({ seedDir: path.join(__dirname, "..", "data", "guides") });
  const calc = refineryIntel.calculateRefinement({
    oreScu: 100,
    yieldPercent: 80,
    stationFeePercent: 5,
    rawSellPerScu: 10,
    refinedSellPerScu: 20,
  });
  assert.strictEqual(calc.refinedScu, 80);
  assert.ok(calc.profitVsRaw > 0);
